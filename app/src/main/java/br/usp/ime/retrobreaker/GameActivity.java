...
public class GameActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        }, 0, Constants.Config.MS_PER_UPDATE * 10);
    }
    @Override
    ...
    private void updateUI() {
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if(Game.State.getGamePaused()) {
                    mReadyTextView.setVisibility(View.VISIBLE);
                } else {
                    mReadyTextView.setVisibility(View.INVISIBLE);
                }

                mScoreTextView.setText(getString(R.string.score) + String.format("%08d", Game.State.getScore()));
                mScoreMultiplierTextView.setText(getString(R.string.multiplier) + Game.State.getScoreMultiplier() + "x");

                /* If the user beats the high score, keep updating the High Score text on the fly
                 * with green text to caught user attention. */
                if(Game.State.getScore() > mHighScore) {
                    mHighScore = Game.State.getScore();
                    mNewHighScore = true;
                ...
                mLivesTextView.setText(getString(R.string.lives) + Game.State.getLives());

                if (Game.State.getGameOver()) {
                    /* Show user score and ask if he wants to play again */
                    showGameOverDialog(Game.State.getScore(), mNewHighScore);
                    ...
                    if(mNewHighScore) {
                        mSharedPrefsEditor.putLong("high_score", mHighScore);
                        mSharedPrefsEditor.commit();
                        mSoundPool.play(mSoundIds.get("victory_fanfare"), Game.State.getVolume(), Game.State.getVolume(), 0, 0, 1.0f);
                    }
